...
Every client has three independent classifications that the agent must determine and
maintain. Understanding the boundary between them is critical -- applying the wrong
classification to the wrong decision produces incorrect outputs.

### The three classifications and what they each control

**KYC client type** (individual-retail, corporate-business, trust-foundation,
financial-institution, pep) controls all due diligence obligations: which documents
are required, which screening checks must be run, which approval roles must sign off,
and which rule atoms in the KYC Policy KnowledgePack apply. The agent always uses
KYC client type -- and only KYC client type -- when looking up policy requirements.
Segment and size never filter rule atoms.

**Segment** (smb, services, markets, banking) identifies the internal business unit
that owns the client relationship. Segment feeds into the risk rating together with
size. The risk rating determines the PR/CR cycle frequency. Segment does not change
document or screening requirements. A segment change is a material change trigger
because it may change the risk rating.

**Size** (large, medium, small) classifies the client by financial scale using
thresholds defined in the organisation's internal policy. Size also feeds into the
risk rating together with segment. A size change is a material change trigger when
it changes the risk rating.

### What derives from these classifications

The **segment + size combination** produces the **client risk rating** (low, medium,
high, or EDD-mandatory) via the risk rating matrix in the KYC Policy KnowledgePack.

The **risk rating** determines the **PR/CR cycle frequency** -- how often the review
must be conducted.

The **KYC client type + jurisdiction + risk level** determines the **due diligence tier**
(SDD, standard, EDD) -- what the review must assess.

These are two separate risk concepts. The segment-size risk rating and the KYC due
diligence tier must always be recorded and shown to the reviewer separately. They must
never be merged into a single label.

### What the agent must do with these classifications in every review

Determine all three classifications from available evidence at the start of Phase 1
Step A (triage). Record the source and confidence of each.

Derive the risk rating from segment and size using the risk rating matrix. Record the
derived rating and compare against the prior review.

Determine the PR/CR cycle frequency from the risk rating. Record whether the current
review is on time, overdue, or early.

During change comparison, check all three classifications for changes. A KYC client
type change is always blocking. A segment or size change that produces a risk rating
change is a material change that must be assessed and documented.

When answering policy questions about document or screening requirements, use KYC
client type only. If a reviewer asks about segment-specific requirements, clarify
that document requirements are governed by KYC client type and ask which type applies.

For the full classification framework including risk rating derivation logic, change
detection protocol, and how to record all three classifications in the client record,
see references/client-classification-framework.md.
...
- references/client-classification-framework.md -- size, segment, KYC type boundaries and risk rating derivation
