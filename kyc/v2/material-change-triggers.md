...
analysis must be rerun under the new type before the review can be concluded. Document
the evidence that supports the reclassification.

### Segment Change

A client has moved from one segment to another (for example from SMB to Services as
their business has grown). Steps to assess:

1. Record the prior segment and the current segment with their sources.
2. Derive the risk rating under the new segment using the risk rating matrix in the
   KYC Policy KnowledgePack, holding size constant.
3. Compare the new risk rating against the prior risk rating.
4. If risk rating changed: record as a risk rating material change. Determine the new
   PR/CR cycle frequency. If the new cycle is shorter (higher risk), flag whether any
   reviews have been missed under the new frequency.
5. If risk rating unchanged despite the segment change: record as a non-blocking
   material change for documentation purposes.

Source for segment: onboarding form and internal records. If the segment shown on the
onboarding form differs from what the relationship manager's current assignment shows,
flag as a data quality issue for reviewer confirmation.

### Size Change

A client's size classification has moved between bands (for example from medium to
large). Same assessment steps as segment change above, holding segment constant.

Source for size: Moodys where available, then audited financial statements, then Orbis
financial extracts. If size cannot be determined from available financial data, record
as not assessed and flag for reviewer confirmation.

### Simultaneous Segment and Size Change

If both segment and size have changed, derive the new risk rating from the new
combination together. Do not assess each independently -- the combined effect on the
risk rating is what matters.

### Review Cycle Change Resulting from Risk Rating Change

If the risk rating change produces a different review cycle frequency, record:

The prior cycle frequency and the new cycle frequency.
Whether any reviews have been missed or would have been missed if the new frequency
had applied retrospectively.
The date the next review is now due under the new frequency.

A review that is overdue under the new cycle is a non-blocking gap unless the overdue
period is significant (more than half the cycle period), in which case escalate to the
reviewer for a decision on whether to conduct an immediate additional review.
